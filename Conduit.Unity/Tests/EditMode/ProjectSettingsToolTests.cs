...
public sealed class ProjectSettingsToolTests
{
    ...
    public void MatcherCombinesTokenAndSeparatorFreeMatches()
    {
        ...
        Assert.That(
            ProjectSettingsTool.Match(registry.Settings, "qual pc async time").Single().Key,
            Is.EqualTo("quality_settings.quality_levels.pc.async_asset_upload.time_slice")
        );
        Assert.That(
            ProjectSettingsTool.Match(
        ...
        Assert.That(
            ProjectSettingsTool.Match(registry.Settings, "Foo Bar")
    ...
    public void PersistencePreservesOperationEmptyValueAndPreviousValue()
    {
        OperationPersistence.SaveActiveOperation(
            ...
            {
                request_id = "project-settings-persistence",
                command_type = BridgeCommandTypes.ProjectSettings,
                target = "test_project_settings.empty_value",
                snippet = string.Empty,
                args = new[] { "set" },
                project_setting_previous = "before",
            },
        ...
        Assert.That(restored.project_setting_previous, Is.EqualTo("before"));
        Assert.That(restored.is_restored, Is.True);
    }
    ...
    public void RestoredSetConfirmsTheAppliedValueWithoutReplayingIt()
    {
        ...
        string result = ProjectSettingsTool.Execute(
            new PendingOperationState
            {
                request_id = "restored-project-settings",
                command_type = BridgeCommandTypes.ProjectSettings,
                target = "test_project_settings.alpha",
                snippet = "replayed_value",
                args = new[] { "set" },
                project_setting_previous = "alpha",
                is_restored = true,
            }
        );
    ...
    static string Execute(
        ...
        => ProjectSettingsTool.Execute(
            new PendingOperationState
            {
                request_id = "project-settings-test",
                command_type = BridgeCommandTypes.ProjectSettings,
                target = key,
                snippet = value,
                args = new[] { operation },
