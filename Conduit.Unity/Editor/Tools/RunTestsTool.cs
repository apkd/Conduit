...
namespace Conduit
{
    static class RunTestsTool
    {
        ...
        {
            ResetState();
            activeTestFilterPattern = TestNameFilter.Normalize(rawTestFilter);
            if (activeTestFilterPattern is { Length: > 0 })
                filter.groupNames = new[] { TestNameFilter.ToRegexPattern(activeTestFilterPattern) };
        }
        ...
        internal static string BuildTestNameRegexPattern(string testFilter)
            => TestNameFilter.ToRegexPattern(testFilter);
