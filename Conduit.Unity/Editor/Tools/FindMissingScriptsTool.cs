...
namespace Conduit
{
    static class FindMissingScriptsTool
    {
        internal static string FindMissingScripts(string assetPattern)
        {
            ...
            using var pooledHits = ConduitPool.GetPooledList<MissingScriptHit>(out var hits);
        ...
        {
            using var pooledPending = ConduitPool.GetPooledList<Transform>(out var pending);
                ...
                if (count > 0)
                    hits.Add(
                        new(
                            assetPath,
                            assetKind,
                            ConduitHierarchyPathUtility.BuildHierarchyPath(transform),
                            count,
        ...
        readonly struct MissingScriptHit
        {
            internal MissingScriptHit(
                string assetPath,
                string assetKind,
                string objectPath,
