...
namespace Conduit
{
    static class BurstLoopAnalyzer
    {
        internal static bool SupportsNativeLoopAnalysis(string cpu, string[] lines, int start, int end)
        {
        ...
        static bool IsWasmInstruction(string line)
        {
            ...
            var tokenEnd = BurstInstructionParser.ReadTokenEnd(text, 0);
            if (tokenEnd == 0)
                return false;
        ...
        internal static void AnalyzeNativeLoops(
            BurstAsmStats stats,
            ...
            IReadOnlyDictionary<int, List<string>> labelsByInstruction)
        {
            ...
            for (var index = 0; index < instructions.Count; ++index)
            {
                ...
                if ((instruction.IsConditionalBranch
                     ...
                     || BurstInstructionParser.IsReturn(instruction.Mnemonic)
            ...
            foreach (var block in blocks)
            {
                ...
                if (BurstInstructionParser.IsReturn(instruction.Mnemonic) || IsTrap(instruction.Mnemonic))
                {
                    block.ExitPathCount++;
                    continue;
                }
            ...
            for (var index = 0; index < loops.Count; ++index)
            {
                ...
                using var pooledNestedBlocks = ConduitPool.GetPooledSet<int>(out var nestedBlocks);
            ...
            using var pooledRegionBlocks = ConduitPool.GetPooledSet<int>(out var regionBlocks);
            foreach (var loop in loops)
        ...
        static bool IsTrap(string mnemonic) =>
            BurstInstructionParser.BaseMnemonic(mnemonic) is "ud2" or "int3" or "brk" or "hlt";
