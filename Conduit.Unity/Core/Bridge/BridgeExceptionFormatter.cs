...
namespace Conduit
{
    ...
    static class BridgeExceptionFormatter
    {
        ...
        internal static BridgeExceptionInfo ToInfo(Exception exception)
        {
            var effective = UnwrapTargetInvocationException(exception);
            return new()
            {
        ...
        internal static string? TrimCommonLogTail(string? stackTrace)
        {
            if (string.IsNullOrWhiteSpace(stackTrace))
                return stackTrace;
