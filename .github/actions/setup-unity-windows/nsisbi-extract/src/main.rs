...
// only commands that change output-directory state or emit a file affect payload reconstruction.
// all other compiled installer commands can be ignored without interpreting the NSIS VM.
...
impl Args {
    fn parse() -> Result<Self> {
        ...
        // NSIS paths use inconsistent casing; normalize once to keep every per-file check cheap.
        for filters in [
    ...
    fn includes(&self, path: &Path) -> bool {
        ...
        // force-includes rescue prerequisites nested inside a broadly excluded temporary directory.
...
// compiled strings can contain variables; Unity's Unicode form encodes them as in-band controls.
fn read_string(block: &[u8], unicode: bool, position: u32) -> Result<String> {
    ...
    for _ in 0..0xffff {
        ...
        match character {
            0 => break,
            // shell and language codes carry one additional encoded word.
            0x01 | 0x02 => offset += 2,
            0x03 => {
...
fn safe_relative_path(path: PathBuf) -> Option<PathBuf> {
    // unresolved installer variables and traversal components have no safe output interpretation.
...
fn decode_chunk(input: &[u8]) -> io::Result<Vec<u8>> {
    ...
    // each NSISBI chunk stores the LZMA properties byte and dictionary size ahead of its raw stream.
...
// the Unity NSIS fork splits one solid data stream into independent LZMA chunks.
// decode one batch ahead so writing or skipping this batch overlaps the next batch's CPU work.
struct SolidReader {
    input: Arc<Mmap>,
...
impl SolidReader {
    ...
    fn schedule_batch(&mut self) -> bool {
        // two chunks per worker amortize scheduling without retaining more than the active and
        // next decoded batches; the bounded channel enforces that memory limit.
        ...
        self.pool.spawn_fifo(move || {
            // indexed parallel iteration preserves installer order despite concurrent decoding.
            let decoded = chunks
                .par_iter()
...
fn extract(args: &Args, pool: Arc<rayon::ThreadPool>) -> Result<()> {
    ...
    // this freshly downloaded installer remains immutable and open for the mapping's lifetime.
    let mmap = Arc::new(unsafe { MmapOptions::new().map(&file)? });
    let (header, data_offset) = read_header(&mmap)?;

    // stream offsets must be visited monotonically; equal offsets represent one payload installed
    // under multiple names, which the value vector reproduces in a single pass.
    ...
    for (offset, paths) in files {
        ...
        // selected Unity payloads are raw inside the outer solid stream; the high bit denotes an
        // additional compression layer whose framing this specialized extractor does not need.
...
fn read_header(installer: &Mmap) -> Result<(Vec<u8>, usize)> {
    // the signed executable stub is variable-sized, so locate the embedded NSIS header by signature.
    ...
    // the NSISBI fork adds an eight-byte data size, then frames both solid streams with u24 lengths.
...
fn collect_files(header: &[u8], args: &Args) -> Result<BTreeMap<u64, Vec<PathBuf>>> {
    ...
    // the Unity script uses $_OUTDIR as a saved directory slot; tracking that assignment together
    // with its CreateDirectory and ExtractFile commands recovers every install destination.
