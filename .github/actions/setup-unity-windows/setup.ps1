...
# range requests saturate hosted-runner bandwidth, while disabled preallocation avoids zero-filling
# the multi-gigabyte installer that exists only until extraction finishes.
...
# current release pages normally expose the installer URL directly; the archive supplies the
# revision for older versions without hard-coding a version-to-revision table.
...
# these jobs exercise x64 development Mono and manifests that omit the optional packages below.
# selecting paths before stream traversal avoids file creation and writes, reducing cold disk work
# and hot-cache transfer; intermediate solid-stream chunks still decode to reach retained files.
$unusedPaths = @(
    "Editor\BugReporter",
    ...
    "Editor\Data\Resources\PackageManager\BuiltInPackages\com.unity.adaptiveperformance",
    "Editor\Data\Resources\PackageManager\BuiltInPackages\com.unity.adaptiveperformance.google.android",
    "Editor\Data\Resources\PackageManager\BuiltInPackages\com.unity.entities",
    "Editor\Data\Resources\PackageManager\BuiltInPackages\com.unity.entities.graphics",
    ...
    "Editor\Data\Resources\PackageManager\BuiltInPackages\com.unity.render-pipelines.core",
    ...
    "Editor\Data\Resources\PackageManager\BuiltInPackages\com.unity.render-pipelines.universal-config",
    "Editor\Data\Resources\PackageManager\BuiltInPackages\com.unity.rendering.denoising",
    "Editor\Data\Resources\PackageManager\BuiltInPackages\com.unity.shadergraph",
...
# package samples are never imported by these fixture manifests and form a large cross-package tree.
$extractorArguments += "--exclude-component", "Samples~"

# the redistributable is the sole prerequisite retained from NSIS's temporary plugin directory;
# force-includes intentionally take precedence over the directory-wide exclusion.
...
# symbols, bundled package archives, and foreign-platform libraries cannot affect these Windows jobs.
...
# keep the redistributable inside the cached tree so its first creator can seed app-local DLLs.
...
if (Test-Path $packageManagerEditor) {
    # keep the manifest used to enumerate built-ins; archives for omitted packages are never requested.
