...
public sealed class UnityProjectOperationsPolicyTests
{
    ...
    {
        await Assert.That(
            UnityStatusPolicy.ShouldReportReachableStatus(Failure(SendTimedOut, commandSent: false))
        ).IsTrue();
        await Assert.That(
            UnityStatusPolicy.ShouldReportReachableStatus(Failure(StartAckTimedOut, commandSent: true))
        ).IsTrue();
        await Assert.That(
            UnityStatusPolicy.ShouldReportReachableStatus(Failure(StartAckDisconnected, commandSent: true))
        ).IsFalse();
        await Assert.That(
            UnityStatusPolicy.ShouldReportReachableStatus(Failure(ResultTimedOut, commandSent: true))
        ).IsTrue();
    }
    ...
    {
        await Assert.That(
            UnityStatusPolicy.ShouldUseProbeExecutionForStatus(BridgeClientResult.Connected(handshake))
        ).IsFalse();
        await Assert.That(
            UnityStatusPolicy.ShouldUseProbeExecutionForStatus(
                BridgeClientResult.Success(handshake, ToolExecutionResult.Success("{}"))
            )
        ).IsTrue();
    }
